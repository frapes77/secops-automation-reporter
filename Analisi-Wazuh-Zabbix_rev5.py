...
def fetch_wazuh_alerts(username, password, target_host=None, baseline_date_str=None, days_duration=30,
                       min_level_num=10):
    """Estrae gli alert da Wazuh a ritroso applicando il filtro esatto .keyword richiesto dall'indexer."""
    try:
        if baseline_date_str:
            end_dt = datetime.strptime(baseline_date_str.strip(), "%d/%m/%Y")
        else:
            end_dt = datetime.now()
        ...
        print(f"[*] Range non valido. Uso ultimi {days_duration} giorni da oggi...")
...
def fetch_zabbix_problems(target_host=None, baseline_date_str=None, days_duration=30, min_prio_zabbix=4):
    ...
    try:
        ...
        time_from = int((datetime.now() - timedelta(days=days_duration)).timestamp())
        time_till = int(datetime.now().timestamp())
        print(f"[*] Data non valida. Uso problemi nati negli ultimi {days_duration} giorni da oggi...")
...
def generate_pdf_report(zabbix_data, wazuh_data, output_pdf, target_host=None, days_back=90, baseline_date_str=None):
        ...
        if zb_rows:
            ...
            zb_table_data = [["Data/Ora Rilevamento", "Host / Apparato", "Descrizione Trigger / Alert", "Priorità"]]
            for row in zb_rows:
                p_weight = row["weight"]
                prio_text = row["prio"]
        ...
        if wz_rows:
            ...
            wz_table_data = [["Data/Ora Rilevamento", "Agent / PC", "Descrizione Vulnerabilità", "Livello/Sev"]]
            for row in wz_rows:
                w_weight = row["weight"]
                lvl_text = row["level"]
            ...
            wz_table = Table(wz_table_data, colWidths=[95, 95, 260, 60])
...
def main():
    ...
    base_date_input = input(
        "Inserisci la DATA DI RIFERIMENTO (GG/MM/AAAA) [INVIO per partire da OGGI e andare a ritroso]: ").strip()
    if base_date_input == "":
        base_date_input = None

    try:
        duration_input = input("Inserisci quanti giorni analizzare A RITROSO da quella data [INVIO per 30]: ").strip()
    ...
    # Estrazione dati a ritroso
    zabbix_problems = fetch_zabbix_problems(target_host, base_date_input, days_duration, min_prio_zabbix)
    wazuh_alerts = fetch_wazuh_alerts(wazuh_user, wazuh_pass, target_host, base_date_input, days_duration,
                                      min_wazuh_lvl)
    ...
    if wazuh_alerts or zabbix_problems or target_host or base_date_input:
        generate_pdf_report(zabbix_problems, wazuh_alerts, output_filename, target_host, days_duration, base_date_input)
    else:
